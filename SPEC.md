...
| Stretch | Note |
| --- | --- |
| **Bounded query tools for the strong model** (`get_series(component, kpi)`, `get_edge(caller, callee)`, ≤ 2 calls, results become new fact IDs) | Makes "the model queries the telemetry" literal; only on escalated cases; watch the 45 s deadline |
...
- **No MCP, and no interface — settled with the judges and in the governing docs.** `docs/scoring.md`: "**There's no interface dimension.** Your agent runs headless and writes files; nobody watches it work. (Track 2 is the visualization track.)" `PARTICIPANT_AGREEMENT.md` §8 assigns "use of MantisGrid AI/MCP" to **Track 2's** focus; Track 1's is "model accuracy, explanability, strength of evaluations, token usage". Slide item 02 is therefore not our criterion. Presentation visuals are still worth building — a figure that explains the engine in one glance (`docs/figures/`) — because Presentation / Demo is 10% and the demo the organizers ask for is a live case plus the evidence file.
...
**Already out:** Streamlit app, video graph animation, Claude API, ridge fix test (stretch), **any MCP server of our own** (Track 1 has no interface dimension and MCP is Track 2's judging focus — checked with the judges), MantisGrid's Track 2 MCP, `log_proxy`, peer z, PageRank, change-event vertices, open-ended multi-turn tool loops (the bounded query tools remain a stretch).
...
**Stretch:** bounded query tools for the strong model, fix-test verifier line, mesh / log signals, thinking ablation.
